...
function fancy_lab_scripts(){
	...
	wp_enqueue_style( 'bootstrap-css', get_template_directory_uri() . '/inc/bootstrap.min.css', array(), '4.4.1', 'all' );
 	...
 	//adiconado após passar o plugin theme check
...
function fancy_lab_config(){
	...
	//para que os futuros arquivos de tradução seja carregados da forma correta
	//carrega o arquivo MO -paramento textdomain e caminho do dir dos arq de tradução
    ...
    //Registra o suporte a temas para um determinado recurso.
	...
	//para quando acessar uma página inexistente
	//é assim que vamos recuperar os títulos das páginas do wordpress
	add_theme_support('title-tag');// página não encontrada
...
//criando sidebar no painel adm - sidebar pra todo site
//não pode estar no gancho after_setup_theme
add_action('widgets_init', 'fancy_lab_sidebars');

function fancy_lab_sidebars() {
	register_sidebar(array(
		//name: título que vai aparecer na área de administração
		'name'			=> esc_html__('Fancy Lab Main Sidebar', 'fancy-lab'),
		//id: tem que ser único. sem caractere especial e espaços
		'id'			=> 'fancy-lab-sidebar-1',
		//descriação curta na area de adm
		'description'	=> esc_html__('Drag and drop your widgets here', 'fancy-lab'),

		//a partir daqui apresentação do widget individulamente
		//definir o elemento vai envolver cada widget da barra lateral
	...
	register_sidebar( 
		array(
			...
			'description'	=> esc_html__('Drag and drop your footer widgets here', 'fancy-lab'),
			'before_widget'	=> '<div id="%1$s" class="widget %2$s widget-wrapper">',
			'after_widget'	=> '</div>',
			'before_title'	=> '<h4 class="widget-title">',
			'after_title'	=> '</h4>',
